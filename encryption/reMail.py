...
from encryption.argonPepper import hashPassword
...
def findPassword(u_id: int, email: str, conn) -> dict:
    """
    임시 비밀번호 발급
    """
    u_logger.info("임시 비밀번호 발급 요청", extra={"action": "findPassword", "u_id": u_id})
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if not mail_sent:
                # conn.rollback()
                u_logger.warning("이메일 발송 실패로 DB 롤백", extra={"action": "findPassword", "u_id": u_id, "err_msg": "Error in sendTempPassword FUNCTION"})
                return {"success": False, "message": "이메일 발송에 실패했습니다"}

            # 이메일 발송 성공 → DB 커밋
            u_logger.info("임시 비밀번호 발급 완료", extra={"action": "findPassword", "u_id": u_id})
        ...
        return {"success": False, "message": "임시 비밀번호 발급 중 서버 오류"}
...
def sendTempPassword(to_email: str, temp_password: str) -> bool:
    ...
    try:
        ...
        # 수신자 이메일 주소가 잘못된 경우
        s_logger.warning("수신자 이메일 주소 오류", extra={"action": "sendTempPassword"})
        return False
