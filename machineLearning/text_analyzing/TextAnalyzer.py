...
class TextAnalyzer:
    def __init__(self):
        ...
        for cat, data in self.entity_data.items():
            for rep, aliases in data.items():
                ...
                for alias in aliases:
                    ...
                    self.all_entity_aliases_normalized.add(alias.lower().replace(" ", ""))
        ...
        self.st_model_ko = SentenceTransformer('snunlp/KR-FinBERT-SC', device="cuda" if self.device == 0 else "cpu")
        self.st_model_en = SentenceTransformer('ProsusAI/finbert', device="cuda" if self.device == 0 else "cpu")
    ...
    def kiwi_tokenizer(self, text):
        ...
        if not hasattr(self, 'dynamic_words'):
    ...
    def _extract_ner(self, content, lang):
        # 1. 100% 사전 기반 추출 (ES에 업로드될 메인 NER)
        ...
        # 2. BERT 기반 신규 후보군 탐색 (ES에 안 올라가고 JSON으로 별도 저장)
        ...
        for ent in ner_raw:
            ...
            if label in ['ORG', 'B-ORG', 'I-ORG', 'corporation']:
                if self._is_valid_company(text, lang, ner_results):
                    candidate_results["company"].append(text)
            elif label in ['PER', 'B-PER', 'I-PER', 'person']:
                if self._is_valid_person(text, lang, ner_results):
                    candidate_results["person"].append(text)

        candidate_results["company"] = list(dict.fromkeys(candidate_results["company"]))
        candidate_results["person"] = list(dict.fromkeys(candidate_results["person"]))
    ...
    def _is_valid_company(self, text, lang, current_ner_results):
        ...
        if text_norm in self.all_entity_aliases_normalized: return False
    ...
    def _is_valid_person(self, text, lang, current_ner_results):
        ...
        if text_raw.lower() in self.person_normalization_dict or text_raw in self.person_normalization_dict:
            return True
    ...
    def process_and_save(self, docs, vectorizer=None):
        actions = []
        new_candidates = {"company": set(), "person": set()}
        ...
        for d in docs:
            try:
                if 'candidates' in d:
                    new_candidates['company'].update(d['candidates']['company'])
                    new_candidates['person'].update(d['candidates']['person'])
    ...
    def _save_candidates(self, new_candidates):
        for cat in ["company", "person"]:
            if not new_candidates[cat]:
                continue

            file_path = os.path.join(self.dict_path, f"{cat}_candidate.json")
            data = {}
            if os.path.exists(file_path):
                try:
                    with open(file_path, 'r', encoding='utf-8') as f:
            ...
            existing_cands_norm = {k.lower().replace(" ", "") for k in data.keys()}

            for term in new_candidates[cat]:
                term_norm = term.lower().replace(" ", "")

                if term_norm not in self.all_entity_aliases_normalized and term_norm not in existing_cands_norm:
                    data[term] = [term.lower()]
                    existing_cands_norm.add(term_norm)
                    updated = True
    ...
    def clear_memory(self):
        gc.collect()
        if self.device == 0: torch.cuda.empty_cache()
