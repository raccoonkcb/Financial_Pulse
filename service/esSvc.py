...
def compare_urls(batch_id: str, lang: str = "ko") -> dict:
    ...
    - 차집합 = 누락 URL
    """
    logger.info("URL 비교 시작", extra={"batch_id": batch_id})

    es           = getEs()
    target_index = NEWS_KO_IDX if lang == "ko" else NEWS_EN_IDX

    log_docs = scan(
        es, index="fp-logs-crawl",
        query={"query": {"term": {"extra.batch_id": batch_id}}},
        size=10000
    )
    crawled_urls = {
        doc["_source"]["extra"]["url"]
        for doc in log_docs
        if "url" in doc["_source"]["extra"]
    }
...
def recollect_missing(batch_id: str, lang: str = "ko") -> dict:
    ...
    if not missing_urls:
        logger.info("누락 URL 없음", extra={"batch_id": batch_id})
...
def get_index_status() -> dict:
    ...
    indices = [
        ...
        {"name": "fp-logs-crawl", "index": "fp-logs-crawl"},
        {"name": "fp-logs-ml",    "index": "fp-logs-ml"},
    ]
    ...
    es.close()
    logger.info("인덱스 현황 조회 완료", extra={"index_cnt": len(result)})
